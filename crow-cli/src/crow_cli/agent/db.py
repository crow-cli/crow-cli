"""
Database schema v4 — back to in-process SQLite (SQLAlchemy), images on disk.

One row = One message. Agent-centric. agent_id = "{session_id}-{idx}" is the
primary key; session_id is the logical parent (multiple agents per session).
...
def get_engine(db_uri: str):
    """SQLite engine with WAL + busy_timeout so multiple processes (crow-cli
    writing, crow-mcp reading) coexist without lock errors."""
...
def search_messages(
    ...
    """BM25 keyword search over message text. Returns {message fields +
    session_id/agent_idx} dicts, best match first."""
    idx = {}
    with Session(engine) as db:
        for a in db.query(Agent).all():
    ...
    with engine.connect() as conn:
        hits = conn.execute(
            text(
                "SELECT rowid FROM messages_fts WHERE messages_fts MATCH :q "
                "ORDER BY bm25(messages_fts) LIMIT :lim"
            ),
            {"q": match, "lim": limit * 4},
        ).fetchall()
    with Session(engine) as db:
