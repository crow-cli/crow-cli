...
- [x] mcpServers round trip through sqlite (CRITICAL PATH, user-mandated).
      ...
      crow-cli-taskmaster.
- [ ] ONE `task` MCP tool (on the crow MCP server, separate process):
      ...
      (session/resume when not live — we do NOT emit history to the
      ...
      Response: per-session status (session_id, success|error,
      error_message) rendered as a string. Verify: FastMCP prototype
      proves the schema the model sees + parse round trip (the two item
      types have OVERLAPPING fields — needs an explicit discriminator,
      prove it in the prototype).
- [ ] Kill the in-process delegate: launch_delegate/_run_subagent/
      ...
      ACP client.
- [ ] Subagent transport: spawn `python -m crow_cli.agent.main`
      subprocess (frozen builds: `crow` acp subcommand — the proven
      CrowClient.spawn_agent pattern in src/crow_cli/client/main.py);
      drive it: initialize → session/new(cwd, mcpServers FROM THE SQLITE
      ROUND TRIP) → session/prompt; session/cancel for CancelTurn.
      Watcher owns the child's PromptResponse future; on resolution —
      STATE FIRST (tasks row + task_deliveries row), then signal.
- [ ] Task state in sqlite: `tasks` + `task_deliveries` tables (additive
...
- [ ] The wake experiment (answers "how did the current version make the
      ...
      request. Prove it with RecordingClient first, then eyeball a real
      client.
...
- [ ] Session context for the task tool: the MCP server process must
      know WHICH session's mcpServers/task state to use. Decide + test
      (candidate: env injection when the agent spawns its stdio MCP
      servers per session; the agent controls that spawn).
...
- [ ] E2E live LLM (qwen3.8-max-preview; llamacpp host coast-after-3 is
      ...
      ways; full suite green (all tiers mandatory).
