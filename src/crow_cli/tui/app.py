...
from crow_cli.tui.settings import Schema, Settings
...
class CrowApp(App, inherit_bindings=False):
    ...
    def system_notify(
        ...
        icon_path = str(files("crow_cli.tui.data").joinpath("images/crow.png"))
    ...
    def _apply_ansi_theme(self) -> None:
        """Match the rich ANSI render theme to the active UI theme."""
        theme = self.settings.get("ui.theme", str)
        self.ansi_theme_dark = (
            SHADES_OF_PURPLE_TERMINAL_THEME
            if theme == "shades-of-purple"
            else DRACULA_TERMINAL_THEME
        )
    ...
    async def on_load(self) -> None:
        ...
        self.register_theme(SHADES_OF_PURPLE)
